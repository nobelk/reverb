...
Goal: give operators a CLI and a graphical surface for the workflows that
today require `curl` or custom Go code.
...
- [ ] Create `nobelk/reverb-ui` sibling repo. Single-page app surfacing:
      hit-rate by namespace over time, top sources by entry count, entry
      browser with filters, test-query box that runs a `lookup` and shows
      tier + similarity + lineage.
- [ ] In the main repo, add `pkg/server/http/admin/` that serves the
      compiled UI bundle via `embed.FS` at `/_admin`. Bundle is checked
      in as a build artifact, regenerated by a `make admin-ui` target
      that pulls from the sibling repo's release tag.
- [ ] Add a `WithAdminUI()` option on the standalone binary; ship
      disabled-by-default until 1.0 (per principle 4).
